...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["profile_file"])) {
    ...
    if (move_uploaded_file($file["tmp_name"], $target_file_path)) {
        ...
        try {
            // mengambil foto lama untuk dihapus (berdasarkan nama)
            $sql_old = "SELECT profile_photo FROM users WHERE user_id = ?";
            $stmt_old = $mysqli->prepare($sql_old);
            $stmt_old->bind_param("i", $user_id);
            $stmt_old->execute();
            $result_old = $stmt_old->get_result();
            $old_photo_row = $result_old->fetch_row(); // mengambil baris pertama
            $old_photo = $old_photo_row ? $old_photo_row[0] : null; // mengambil kolom pertama

            // mengupdate foto baru berdasarkan nama
